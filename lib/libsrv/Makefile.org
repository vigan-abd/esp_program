...
SPLINT  = $(QUOTE)/usr/bin/splint$(QUOTE)
DOXYGEN = $(QUOTE)/usr/bin/doxygen$(QUOTE)
...
debug:    CFLAGS += -DDEBUG -O0 -g3 -I./include
debug:    $(OBJ_DIR) $(LST_DIR) $(EXE_DIR) $(EXE_DIR)/$(LIBNAME).out

release:  CFLAGS += -DNDEBUG -O3 -I./include
release:  $(OBJ_DIR) $(LST_DIR) $(EXE_DIR) $(EXE_DIR)/$(LIBNAME).out
...
# Link
$(EXE_DIR)/$(LIBNAME).out: $(C_OBJS) $(S_OBJS)
	@echo "Linking target: $@"
	$(AR) rs $(EXE_DIR)/$(LIBNAME).a $(C_OBJS) $(S_OBJS) $(LIBS) 

splint:
	INCLUDEPATHS += -I/usr/src/linux-headers-4.10.0-28/arch/avr32/include
